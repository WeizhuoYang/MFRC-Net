"""
10 fold cross-validation classification analysis of BCI comp Ⅳ-2a and SHU dataset
"""
import numpy as np
import torch
import sys
...
debug = False
def cv(datasetId = None, network = None, nGPU = None,batchSize=16,numEpochs=200,cllr=0.001,Lambda=0.0005,Alpha=0.05,subTorun=None,centerLossEnable=None):
    datasetId = datasetId or 0
    network =network or 'MFRCNet'
    nGPU = nGPU or 0
    ...
    if datasetId == 1:
        config['modelArguments'] = {'nChan': 20, 'nTime': 1000, 'dropoutP': 0.5,
                                    'nBands':9, 'temporalLayer': 'VarLayer',
        ...
        config['modelArguments'] = {'nChan': 32, 'nTime': 1000, 'dropoutP': 0.5,
                                    'nBands': 9,  'temporalLayer': 'LogVarLayer',
                                    'nClass': 2, 'doWeightNorm': True}
    ...
    config['modelTrainArguments'] = {'stopCondi':  {'c': {'Or': {'c1': {'MaxEpoch': {'maxEpochs': 1500, 'varName' : 'epoch'}},
          ...
          'bestVarToCheck': 'valInacc', 'continueAfterEarlystop':True,'lr': 1e-3,'cllr':cllr,'Lambda':Lambda,'Alpha':Alpha}
    ...
    n = 'batch' + str(batchSize) + '-' + 'numEpoch' + str(numEpochs) + '-' + \
        str(centerLossEnable)+ 'cllr' + str(cllr) + '-'+ 'Lambda' + str(Lambda) + '-'+ 'Alpha' + str(Alpha)
    randomFolder = str(time.strftime("%Y-%m-%d--%H-%M", time.localtime())) + '-' + str(
        random.randint(1, 1000)) + '-' + n
    ...
    fetchData(os.path.dirname(config['inDataPath']), datasetId)
    ...
    if len(data.labels[0])>4:
        idx = [i for i, x in enumerate(data.labels)]# if x[4] == '0' or x[4] == '1']
...
if __name__ == '__main__':
    ...
    cv(datasetId=0, network='MFRCNet', nGPU='0', batchSize=16,numEpochs=200,cllr=0.001,
       Lambda=0.0005,Alpha=0.05,subTorun=subTorun,centerLossEnable=True)